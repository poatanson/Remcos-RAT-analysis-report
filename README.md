...
* **환경 변수 생성**: VBS가 반복문으로 환경 변수 `P1`~`P37`을 생성하고, 각 변수에
  Base64로 인코딩된 **PE 파일(.NET 어셈블리로 추정)**의 조각을 나누어 저장함.
  동적 분석에서 `P1`이 PE 헤더로 시작함을 확인함 (6.4절).
* **추출된 WMI 스크립트:**
```vbscript
' ==========================================================
...
- `P1`의 앞 16자를 디코딩한 결과가 `4D-5A-90-00-03-00-00-00-04-00-00-00`으로,
  PE 파일의 `MZ` 헤더(DOS 헤더 시작부)임을 확인함
  (`[BitConverter]::ToString([Convert]::FromBase64String($env:P1.Substring(0,16)))`)
- 로더가 `[Reflection.Assembly]::Load($b)`로 이 데이터를 올리므로 관리형 .NET 어셈블리로 판단됨
  (네이티브 PE는 이 API로 로드할 수 없음). 전체 조각을 복원한 CLR 헤더 확인은 미수행
- 변수는 사용자 환경 변수로 남아 있으므로 감염 흔적으로 활용 가능함
- 화면에는 4개만 보이며, 37개라는 수는 커맨드라인의 `$env:P1+...+$env:P37` 참조에서 확인한 값임
- 복원한 파일에서 `OtnmpxnddVnptbN.mpxnddVn` 클래스의 `Otnmpxn` 메서드를 확인함
  (`ReflectionOnlyLoadFrom`으로 메타데이터만 읽음, 코드 실행 없음). 이로써 환경 변수
  `P1`~`P37`이 커맨드라인이 호출하는 .NET 어셈블리의 조각임을 확정함
- 복원 시 사용자(User) 범위 환경 변수를 읽어 파일이 정상 복원되므로, 변수는 사용자 환경(레지스트리)에
  영구 저장되어 있음
