...
>
> **안내:** 이 저장소에는 악성 샘플(원본 VBS, 복원한 .NET 페이로드, 내장 리소스 등)이 **포함되어 있지 않습니다.** 파일은 해시(SHA-256)로만 식별하며, 분석 내용과 IOC는 방어·연구 목적으로 공개합니다.
...
- **룰 파일**: [`proc_creation_win_powershell_reflection_assembly_load_b64_hidden.yaml`](proc_creation_win_powershell_reflection_assembly_load_b64_hidden.yaml)
